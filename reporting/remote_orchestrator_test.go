...
func TestDefaultObjectTypes(t *testing.T) {
	...
	if len(types) == 0 {
		t.Error("Expected at least some object types")
	}

	// Check that all core types are present
	for _, expectedType := range expectedCoreTypes {
		found := false
		...
		if !found {
			t.Errorf("Expected core object type %s not found", expectedType)
		}
	}
}
...
func TestRequiredFields(t *testing.T) {
	// Test that required fields are defined for core object types
	coreTypes := []string{
		"productOffering",
		"productSpecification",
		"productOfferingPrice",
	...
	// Test that RequiredFieldsForAllObjects is defined
	if len(RequiredFieldsForAllObjects) == 0 {
		t.Error("RequiredFieldsForAllObjects should not be empty")
	}
}

func TestRequiredRelatedPartyRoles(t *testing.T) {
	// Test that required related party roles are defined for core object types
	coreTypes := []string{
		"productOffering",
		"productSpecification",
		"productOfferingPrice",
	...
	// Test that DoNotRequireRelatedParties is defined
	if len(DoNotRequireRelatedParties) == 0 {
		t.Error("DoNotRequireRelatedParties should not be empty")
	}
}
