...
func TestRoutesMapCompleteness(t *testing.T) {
	...
	// Test that core object types have required fields defined
	coreTypes := []string{
		"productOffering",
	...
	for _, objType := range coreTypes {
		if _, exists := RequiredFields[objType]; !exists {
			t.Errorf("Core object type %s missing from RequiredFields map", objType)
		}
	}

	// Test that core object types have related party roles defined (except those that don't require them)
	coreTypesWithRelatedParty := []string{
		"productOffering",
		"productSpecification",
		"productOfferingPrice",
		"customer",
