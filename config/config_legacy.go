...
var proConfigLegacy = &ConfigLegacy{
...
var dev2ConfigLegacy = &ConfigLegacy{
...
var sbxConfigLegacy = &ConfigLegacy{
...
var lclConfigLegacy = &ConfigLegacy{
...
var isbeConfigLegacy = &ConfigLegacy{
...
func DefaultConfig(where Environment, internal bool, usingBAEProxy bool) *ConfigLegacy {
	return NewDefaultConfig(where, internal, usingBAEProxy, false)
}

func NewDefaultConfig(where Environment, internal bool, usingBAEProxy bool, proxyEnabled bool) *ConfigLegacy {
	var conf *ConfigLegacy

	switch where {
	case DOME_PRO:
		conf = proConfigLegacy
	case DOME_DEV2:
		conf = dev2ConfigLegacy
	case DOME_SBX:
		conf = sbxConfigLegacy
	case DOME_LCL:
		conf = lclConfigLegacy
	case ISBE_EVIDENCE:
		conf = isbeConfigLegacy
	default:
...
// LoadConfigLegacy initializes and returns a Config struct based on the provided parameters.
...
func (c *ConfigLegacy) InitUpstreamHosts(hosts map[string]string) {
...
func (c *ConfigLegacy) SetUpstreamHost(resourceName string, host string) {
	c.internalUpstreamPodHosts.Store(resourceName, host)
}

// GetUpstreamHost provides a typed method for the upstream hosts map
func (c *ConfigLegacy) GetUpstreamHost(resourceName string) string {
...
func (c *ConfigLegacy) GetAllUpstreamHosts() map[string]string {
...
func (c *ConfigLegacy) UpstreamHostAndPathFromResource(resourceName string) (string, error) {

	if c.Environment == ISBE_EVIDENCE {
